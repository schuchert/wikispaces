...
[[Katas|<--Back]]
# Background
The [Dijkstra](http://en.wikipedia.org/wiki/Shunting_yard_algorithm|Shunting Yard Algorithm]], written by [[http://en.wikipedia.org/wiki/Edsger_Dijkstra), converts an infix expression into a post-fix expression. For example, the expression:
^
|---|---|---|
|$$ 3 + 4 $$|becomes|$$ 3\;4\;+ $$|
...
**The Test**
Note: Occasionally I experiment with different ways of writing unit tests with JUnit. This is one such example where I attempt to simulate the [cucumber](http://www.martinfowler.com/bliki/BusinessReadableDSL.html|domain specific language]] used by [[http://cukes.info/).
```java
